...
function doExternalLink(url) {
    window.open(url, '_blank').focus();
}

// when someone clicks a link in the content
function doLink(targetDoc, targetPtr, useNewScreen, viewCompleteTopic) {
...
function getNoteResultHandler(notes) {
    ...
    if (notes.length == 1) {
        ...
        if ($("#iframe-main").contents().find(editDivIdJQuery).length > 0) {
            ...
            html += "<button onclick=\"top.updateNote('" + note.TargetDoc + "', '" + note.TargetPtr + "', '" + note.Id + "')\">Save Changes</button>";
        ...
        for (var i = 0; i < notes.length; i++) {
            ...
            html += "<div>";
            //html += "<a href='#' onclick=\"updateNote('" + note.TargetDoc + "', '" + note.TargetPtr + "', '" + note.Id + "')\"><img src='images/btn-save.gif' alt='Save Changes' border='0'></a>";
            html += "<button onclick=\"top.updateNote('" + note.TargetDoc + "', '" + note.TargetPtr + "', '" + note.Id + "', '" + i + "')\">Save Changes</button>";
            //html += "<a href='#' onclick=\"deleteNote('" + note.TargetDoc + "', '" + note.TargetPtr + "', '" + note.Id + "')\"><img src='images/btn-delete.gif' alt='Delete' border='0'></a>";
        ...
        $("#cluetip").remove();
        $("#iframe-main").contents().find("#" + anchorId.replace(/\./g, "\\.")).cluetip({
            activation: 'click', sticky: true, local: true, hideLocal: true, clickThrough: true, topOffset: 0, leftOffset: 25, closePosition: 'title', cluezIndex: 99
...
$(document).ready(function () {
    $(".noteTip").cluetip({
        activation: 'click', sticky: true, local: true, hideLocal: true, topOffset: 0, leftOffset: 0, closePosition: 'title',
        onShow: function () {
