...
namespace dk.nita.saml20.protocol
{
    ...
    public class Saml20LogoutHandler : Saml20AbstractEndpointHandler
    {
        ...
        public Saml20LogoutHandler()
        {
            ...
            {
                RedirectUrl =
                    ConfigurationInstance<SAML20FederationConfig>.GetConfig().ServiceProvider.LogoutEndpoint.RedirectUrl;
                ErrorBehaviour =
                    ConfigurationInstance<SAML20FederationConfig>.GetConfig().ServiceProvider.LogoutEndpoint.
        ...
        private void HandleSOAP(HttpContext context, Stream inputStream)
        {
            ...
            var builder = new HttpArtifactBindingBuilder(context);
            SAML20FederationConfig config = ConfigurationInstance<SAML20FederationConfig>.GetConfig();
            ...
            if (parser.IsArtifactResolve())
            {
                ...
                if (!parser.CheckSamlMessageSignature(idp.metadata.Keys))
                {
                    ...
                    AuditLogging.logEntry(Direction.UNDEFINED, Operation.ARTIFACTRESOLVE,
                                          "Signature could not be verified", parser.SamlMessage);
                }
            ...
            else if (parser.IsArtifactResponse())
            {
                ...
                if (status.StatusCode.Value != Saml20Constants.StatusCodes.Success)
                {
                    AuditLogging.logEntry(Direction.UNDEFINED, Operation.ARTIFACTRESOLVE,
                                          string.Format(
                                              "Unexpected status code for artifact response: {0}, expected 'Success', msg: {1}",
                ...
                if (parser.ArtifactResponse.Any.LocalName == LogoutRequest.ELEMENT_NAME)
                {
                    if (Trace.ShouldTrace(TraceEventType.Information))
                        Trace.TraceData(TraceEventType.Information,
                                        string.Format(Tracing.LogoutRequest, parser.ArtifactResponse.Any.OuterXml));
                    ...
                    IDPEndPointElement destination =
                        DetermineEndpointConfiguration(SAMLBinding.REDIRECT, endpoint.SLOEndpoint,
                                                       endpoint.metadata.SLOEndpoints());
            ...
            else if (parser.IsLogoutReqest())
            {
                if (Trace.ShouldTrace(TraceEventType.Information))
                    Trace.TraceData(TraceEventType.Information,
                                    string.Format(Tracing.LogoutRequest, parser.SamlMessage.OuterXml));
        ...
        private void TransferClient(IDPEndPoint endpoint, HttpContext context)
        {
            ...
            if (destination.Binding == SAMLBinding.POST)
            {
                ...
                if (Trace.ShouldTrace(TraceEventType.Information))
                    Trace.TraceData(TraceEventType.Information,
                                    string.Format(Tracing.SendLogoutRequest, "POST", endpoint.Id,
            ...
            if (destination.Binding == SAMLBinding.REDIRECT)
            {
                ...
                if (Trace.ShouldTrace(TraceEventType.Information))
                    Trace.TraceData(TraceEventType.Information,
                                    string.Format(Tracing.SendLogoutRequest, "REDIRECT", endpoint.Id, redirectUrl));
            ...
            if (destination.Binding == SAMLBinding.ARTIFACT)
            {
                if (Trace.ShouldTrace(TraceEventType.Information))
                    Trace.TraceData(TraceEventType.Information,
                                    string.Format(Tracing.SendLogoutRequest, "ARTIFACT", endpoint.Id, string.Empty));
        ...
        private void HandleResponse(HttpContext context)
        {
            ...
            {
                AuditLogging.logEntry(Direction.IN, Operation.LOGOUTRESPONSE,
                                      string.Format("Unsupported request type format, type: {0}",
            ...
            {
                AuditLogging.logEntry(Direction.IN, Operation.LOGOUTRESPONSE,
                                      string.Format("Unexpected status code: {0}, msg: {1}", status.StatusCode.Value,
                                                    message));
                HandleError(context, status);
                return;
            }
        ...
        private void HandleRequest(HttpContext context)
        {
            ...
            IDPEndPointElement destination =
                DetermineEndpointConfiguration(SAMLBinding.REDIRECT, idpEndpoint.SLOEndpoint,
                                               idpEndpoint.metadata.SLOEndpoints());

            //Fetch config object
            var config = ConfigurationReader.GetConfig<SAML20FederationConfig>();
            ...
            if (context.Request.RequestType == "GET") // HTTP Redirect binding
            {
                ...
                if (!parser.VerifySignature(metadata.GetKeys(KeyTypes.signing)))
                {
                    AuditLogging.logEntry(Direction.IN, Operation.LOGOUTREQUEST,
                                          "Invalid signature, msg: " + parser.Message);
                    HandleError(context, Resources.SignatureInvalid);
                    return;
                }
            ...
            else if (context.Request.RequestType == "POST") // HTTP Post binding
            {
                ...
                {
                    AuditLogging.logEntry(Direction.IN, Operation.LOGOUTREQUEST,
                                          "Signature not present, msg: " + parser.Message);
                    HandleError(context, Resources.SignatureNotPresent);
                }
                if (!parser.CheckSignature())
                {
