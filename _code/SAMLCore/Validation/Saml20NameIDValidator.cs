...
namespace dk.nita.saml20.Validation
{
    ...
    {
        private readonly Saml20EncryptedElementValidator EncElemValidator = new Saml20EncryptedElementValidator();

        #region ISaml20NameIDValidator Members
        ...
        public void ValidateNameID(NameID nameID)
        {
            ...
            if (nameID.Format == Saml20Constants.NameIdentifierFormats.Email)
            {
                ...
                {
                    throw new Saml20FormatException(
                        "Value of NameID is not a valid email address according to the IETF RFC 2822 specification", fe);
                }
                catch (IndexOutOfRangeException ie)
                {
                    throw new Saml20FormatException(
                        "Value of NameID is not a valid email address according to the IETF RFC 2822 specification", ie);
                }
            }
            ...
            else if (nameID.Format == Saml20Constants.NameIdentifierFormats.Transient)
            {
                if (!Saml20Utils.ValidateRequiredString(nameID.Value))
                    throw new Saml20FormatException(
                        "NameID with Transient Format attribute MUST contain a Value that contains more than whitespace characters");
