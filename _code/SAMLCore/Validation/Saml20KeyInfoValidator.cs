        ...
        public void ValidateKeyInfo(SubjectConfirmationData subjectConfirmationData)
        {
            if (subjectConfirmationData == null)
                throw new Saml20FormatException(
                    "SubjectConfirmationData cannot be null when KeyInfo subelements are required");

            if (subjectConfirmationData.AnyElements == null)
                throw new Saml20FormatException(
                    String.Format("SubjectConfirmationData element MUST have at least one {0} subelement",
                                  KeyInfo.ELEMENT_NAME));
            ...
            foreach (XmlElement element in subjectConfirmationData.AnyElements)
            {
                ...
                if (!element.HasChildNodes)
                    throw new Saml20FormatException(
                        String.Format("{0} subelement of SubjectConfirmationData MUST NOT be empty",
                                      KeyInfo.ELEMENT_NAME));
            }
            ...
            if (!keyInfoFound)
                throw new Saml20FormatException(
                    String.Format("SubjectConfirmationData element MUST contain at least one {0} in namespace {1}",
                                  KeyInfo.ELEMENT_NAME, Saml20Constants.XMLDSIG));
        }
    }
}