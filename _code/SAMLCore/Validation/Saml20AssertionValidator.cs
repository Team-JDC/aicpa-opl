    ...
    internal class Saml20AssertionValidator : ISaml20AssertionValidator
    {
        ...
        public void ValidateTimeRestrictions(Assertion assertion, TimeSpan allowedClockSkew)
        {
            ...
            if (assertion.Subject != null)
            {
                foreach (object o in assertion.Subject.Items)
                {
                    ...
                    if (
                        !ValidateNotOnOrAfter(subjectConfirmation.SubjectConfirmationData.NotOnOrAfter, now,
                                              allowedClockSkew))
        ...
        private void ValidateAssertionAttributes(Assertion assertion)
        {
            ...
            if (!Saml20Utils.ValidateIDString(assertion.ID))
                throw new Saml20FormatException(
        ...
        private void ValidateConditions(Assertion assertion)
        {
            AuditLogging.logEntry(Direction.IN, Operation.AUTHNREQUEST_POST,
                                  "Validate condition: " + assertion.Issuer.ToString()); ;
            // Conditions are not required
            if (assertion.Conditions == null)
                return;
            ...
            foreach (ConditionAbstract cat in assertion.Conditions.Items)
            {
                ...
                if (cat is ProxyRestriction)
                {
                    ...
                    if (!String.IsNullOrEmpty(proxyRestriction.Count))
                    {
                        ...
                        if (!UInt32.TryParse(proxyRestriction.Count, out res))
                            throw new Saml20FormatException(
                                "Count attribute of ProxyRestriction MUST BE a non-negative integer");
                    }
                ...
                if (cat is AudienceRestriction)
                {
                    ...
                    string match = null;
                    foreach (string allowedUri in _allowedAudienceUris)
                    {
                        AuditLogging.logEntry(Direction.IN, Operation.AUTHNREQUEST_POST,
                                  "Checking allowed URI: " + allowedUri); ;
                    }

                    foreach (string audience in audienceRestriction.Audience)
                    {
                        AuditLogging.logEntry(Direction.IN, Operation.AUTHNREQUEST_POST,
                                  "Checking audience restriction: " + audience); ;
                    ...
                    {
                        string intended = "Intended uris: " + Environment.NewLine +
                                          String.Join(Environment.NewLine, audienceRestriction.Audience.ToArray());
                        string allowed = "Allowed uris: " + Environment.NewLine +
                                         String.Join(Environment.NewLine, _allowedAudienceUris.ToArray());
                        Trace.TraceData(TraceEventType.Verbose, Trace.CreateTraceString(GetType(), "ValidateConditions"),
                                        intended, allowed);
                    }
        ...
        private static void ValidateConditionsInterval(Conditions conditions)
        {
            ...
            if (conditions.NotBefore != null && conditions.NotOnOrAfter != null &&
                conditions.NotBefore.Value >= conditions.NotOnOrAfter.Value)
                throw new Saml20FormatException(
                    String.Format("NotBefore {0} MUST BE less than NotOnOrAfter {1} on Conditions",
                                  Saml20Utils.ToUTCString(conditions.NotBefore.Value),
                                  Saml20Utils.ToUTCString(conditions.NotOnOrAfter.Value)));
        }
