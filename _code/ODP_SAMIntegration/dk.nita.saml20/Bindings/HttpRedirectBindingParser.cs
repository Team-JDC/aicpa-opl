...
namespace dk.nita.saml20.Bindings
{
    ...
    public class HttpRedirectBindingParser
    {
        ...
        private void CreateSignatureSubject(IDictionary<string, string> queryParams)
        {
            ...
            if (_relaystate != null)
                signedQuery.AppendFormat("&{0}=", HttpRedirectBindingConstants.RelayState).Append(
                    queryParams[HttpRedirectBindingConstants.RelayState]);

            if (_signature != null)
                signedQuery.AppendFormat("&{0}=", HttpRedirectBindingConstants.SigAlg).Append(
        ...
        private static string DeflateDecompress(string str)
        {
            ...
            {
                var testStream = new StreamReader(new BufferedStream(stream), Encoding.ASCII);
