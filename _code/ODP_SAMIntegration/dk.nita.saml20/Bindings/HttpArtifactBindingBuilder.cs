...
namespace dk.nita.saml20.Bindings
{
    ...
    public class HttpArtifactBindingBuilder : HttpSOAPBindingBuilder
    {
        ...
        public void RedirectFromLogin(IDPEndPointElement destination, Saml20AuthnRequest request)
        {
            ...
            var index = (Int16) config.ServiceProvider.SignOnEndpoint.endPointIndex;
        ...
        {
            SAML20FederationConfig config = ConfigurationInstance<SAML20FederationConfig>.GetConfig();
            var index = (Int16) config.ServiceProvider.LogoutEndpoint.endPointIndex;
            XmlDocument doc = request.GetXml();
            XmlSignatureUtils.SignDocument(doc, request.Request.ID);
            ArtifactRedirect(destination, index, doc);
        }
        ...
        /// <param name="relayState">The query string relay state value to add to the communication</param>
        private void ArtifactRedirect(IDPEndPointElement destination, Int16 localEndpointIndex,
                                      XmlDocument signedSamlMessage, string relayState)
        {
            SAML20FederationConfig config = ConfigurationInstance<SAML20FederationConfig>.GetConfig();
            ...
            string artifact = ArtifactUtil.CreateArtifact(HttpArtifactBindingConstants.ArtifactTypeCode,
                                                          localEndpointIndex, sourceIdHash, messageHandle);

            _context.Cache.Insert(artifact, signedSamlMessage, null, DateTime.Now.AddMinutes(1),
            ...
            if (Trace.ShouldTrace(TraceEventType.Information))
            {
                Trace.TraceData(TraceEventType.Information,
                                string.Format(Tracing.CreatedArtifact, artifact, signedSamlMessage.OuterXml));
            }
        ...
        {
            var samlDoc = (XmlDocument) _context.Cache.Get(artifactResolve.Artifact);
            ...
            {
                Trace.TraceData(TraceEventType.Information,
                                string.Format(Tracing.RespondToArtifactResolve, artifactResolve.Artifact,
                                              responseDoc.OuterXml));
            }
        ...
        public Stream ResolveArtifact()
        {
            ...
            {
                Trace.TraceData(TraceEventType.Information,
                                string.Format(Tracing.ResolveArtifact, artifact, idpEndPoint.Id, endpointIndex,
        ...
        private IDPEndPoint DetermineIdp(string artifact)
        {
            ...
            if (ArtifactUtil.TryParseArtifact(artifact, ref typeCodeValue, ref endPointIndex, ref sourceIdHash,
                                              ref messageHandle))
            {
