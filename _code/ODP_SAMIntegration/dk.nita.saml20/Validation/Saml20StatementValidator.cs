...
namespace dk.nita.saml20.Validation
{
    internal class Saml20StatementValidator : ISaml20StatementValidator
    {
        ...
        #region ISaml20StatementValidator Members
        ...
        private void ValidateAuthnContext(AuthnContext authnContext)
        {
            ...
            for (int i = 0; i < authnContext.ItemsElementName.Length; ++i)
            {
                switch (authnContext.ItemsElementName[i])
                {
                    ...
                    default:
                        throw new Saml20FormatException(string.Format(
                                                            "Subelement {0} of AuthnContext is not supported",
                                                            authnContext.ItemsElementName[i]));
                }
            }
        ...
        private void ValidateAttributeStatement(AttributeStatement statement)
        {
            ...
            foreach (object o in statement.Items)
            {
                ...
                if (o is SamlAttribute)
                    ...
                    throw new Saml20FormatException(
                        string.Format("Subelement {0} of AttributeStatement is not supported", o.GetType()));
            }
        }
        ...
        private void ValidateAuthzDecisionStatement(AuthzDecisionStatement statement)
        {
            ...
            if (statement.Action == null || statement.Action.Length == 0)
                throw new Saml20FormatException(
                    "At least one Action subelement must be present for an AuthzDecisionStatement element");

            foreach (Action action in statement.Action)
            {
                ...
                if (!Saml20Utils.ValidateRequiredString(action.Namespace))
                    throw new Saml20FormatException(
                        "Namespace attribute of Action element must contain at least one non-whitespace character");

                if (!Uri.IsWellFormedUriString(action.Namespace, UriKind.Absolute))
                    throw new Saml20FormatException(
                        "Namespace attribute of Action element has a value which is not a wellformed absolute uri");
            }
        }
    }
}