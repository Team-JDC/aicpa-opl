...
namespace dk.nita.saml20.protocol
{
    ...
    public class Saml20SignonHandler : Saml20AbstractEndpointHandler
    {
        ...
        {
            _certificate = ConfigurationInstance<FederationConfig>.GetConfig().SigningCertificate.GetCertificate();

            // Read the proper redirect url from config
            try
            {
                RedirectUrl =
                    ConfigurationInstance<SAML20FederationConfig>.GetConfig().ServiceProvider.SignOnEndpoint.RedirectUrl;
                ErrorBehaviour =
                    ConfigurationInstance<SAML20FederationConfig>.GetConfig().ServiceProvider.SignOnEndpoint.
                        ErrorBehaviour.ToString();
            }
            catch (Exception e)
            {
        ...
        protected override void Handle(HttpContext context)
        {
            ...
            if (Array.Exists(context.Request.Headers.AllKeys,
                             delegate(string s) { return s == SOAPConstants.SOAPAction; }))
            {
            ...
            {
                if (ConfigurationInstance<SAML20FederationConfig>.GetConfig().CommonDomain.Enabled &&
                    context.Request.QueryString["r"] == null
                    && context.Request.Params["cidp"] == null)
                {
                    AuditLogging.logEntry(Direction.OUT, Operation.DISCOVER,
                                          "Redirecting to Common Domain for IDP discovery");
                    context.Response.Redirect(
                        ConfigurationInstance<SAML20FederationConfig>.GetConfig().CommonDomain.LocalReaderEndpoint);
                }
                else
                {
        ...
        private void HandleSOAP(HttpContext context, Stream inputStream)
        {
            ...
            if (parser.IsArtifactResolve())
            {
                ...
                if (!parser.CheckSamlMessageSignature(idp.metadata.Keys))
                {
                    ...
                    AuditLogging.logEntry(Direction.IN, Operation.ARTIFACTRESOLVE, "Could not verify signature",
                                          parser.SamlMessage);
                }
                ;
            ...
            else if (parser.IsArtifactResponse())
            {
                ...
                if (status.StatusCode.Value != Saml20Constants.StatusCodes.Success)
                {
                    ...
                    AuditLogging.logEntry(Direction.IN, Operation.ARTIFACTRESOLVE,
                                          string.Format(
                                              "Illegal status for ArtifactResponse {0} expected 'Success', msg: {1}",
                                              status.StatusCode.Value, parser.SamlMessage));
                    return;
                }
                ...
                else
                {
                    AuditLogging.logEntry(Direction.IN, Operation.ARTIFACTRESOLVE,
                                          string.Format(
                                              "Unsupported payload message in ArtifactResponse: {0}, msg: {1}",
                                              parser.ArtifactResponse.Any.LocalName, parser.SamlMessage));
                    HandleError(context,
            ...
            else
            {
                ...
                {
                    AuditLogging.logEntry(Direction.IN, Operation.ARTIFACTRESOLVE,
                                          string.Format("Unsupported SamlMessage element: {0}, msg: {1}",
                                                        parser.SamlMessageName, parser.SamlMessage));
        ...
        {
            AuditLogging.logEntry(Direction.IN, Operation.ARTIFACTRESOLVE, string.Format(context.Response.ToString()));
            ...
            try
            {
                ...
                if (isEncrypted)
                {
                    ...
                    AuditLogging.logEntry(Direction.IN, Operation.ARTIFACTRESOLVE,
                                          string.Format("Decrypted Assertion..."));
                }

                // Check if an encoding-override exists for the IdP endpoint in question
                string issuer = "http://localhost:1564/"; //GetIssuer(assertion);
                AuditLogging.logEntry(Direction.IN, Operation.ARTIFACTRESOLVE, string.Format(issuer));

                IDPEndPoint endpoint = RetrieveIDPConfiguration(issuer);
                AuditLogging.logEntry(Direction.IN, Operation.ARTIFACTRESOLVE, string.Format("Endpoint check..."));
                if (!string.IsNullOrEmpty(endpoint.ResponseEncoding))
                {
                    ...
                    if (encodingOverride.CodePage != defaultEncoding.CodePage)
                    {
                        ...
                        AuditLogging.logEntry(Direction.IN, Operation.ARTIFACTRESOLVE,
                                              string.Format("Override Assertion..."));
                    }
                }
        ...
        {
            object expectedInResponseToSessionState = context.Session[ExpectedInResponseToSessionKey];
        ...
        {
            var decryptedAssertion =
                new Saml20EncryptedAssertion(
                    (RSA)
                    ConfigurationInstance<FederationConfig>.GetConfig().SigningCertificate.GetCertificate().PrivateKey);
        ...
        protected virtual void PreHandleAssertion(HttpContext context, XmlElement elem, IDPEndPoint endpoint)
        {
            ...
            if (endpoint != null && endpoint.SLOEndpoint != null &&
                !String.IsNullOrEmpty(endpoint.SLOEndpoint.IdpTokenAccessor))
            {
                var idpTokenAccessor =
                    Activator.CreateInstance(Type.GetType(endpoint.SLOEndpoint.IdpTokenAccessor, false)) as
                    ISaml20IdpTokenAccessor;
        ...
        internal static IEnumerable<AsymmetricAlgorithm> GetTrustedSigners(ICollection<KeyDescriptor> keys,
                                                                           IDPEndPoint ep)
        {
            if (keys == null)
        ...
        private void DoLogin(HttpContext context, Saml20Assertion assertion)
        {
            ...
            if (Trace.ShouldTrace(TraceEventType.Information))
            {
                Trace.TraceData(TraceEventType.Information,
                                string.Format(Tracing.Login, assertion.Subject.Value, assertion.SessionIndex,
                                              assertion.Subject.Format));
            }
            AuditLogging.logEntry(Direction.IN, Operation.LOGIN,
                                  "Subject: " + assertion.Subject.Value + " NameIDFormat: " + assertion.Subject.Format);
        ...
        private void TransferClient(IDPEndPoint idpEndpoint, Saml20AuthnRequest request, HttpContext context)
        {
            ...
            IDPEndPointElement destination =
                DetermineEndpointConfiguration(SAMLBinding.REDIRECT, idpEndpoint.SSOEndpoint,
                                               idpEndpoint.metadata.SSOEndpoints());
            ...
            {
                Trace.TraceData(TraceEventType.Information,
                                string.Format(Tracing.SendAuthnRequest, Saml20Constants.ProtocolBindings.HTTP_Redirect,
                                              idpEndpoint.Id));
                ...
                AuditLogging.logEntry(Direction.OUT, Operation.AUTHNREQUEST_REDIRECT,
                                      "Redirecting user to IdP for authentication", builder.Request);
            ...
            {
                Trace.TraceData(TraceEventType.Information,
                                string.Format(Tracing.SendAuthnRequest, Saml20Constants.ProtocolBindings.HTTP_Post,
                                              idpEndpoint.Id));
            ...
            {
                Trace.TraceData(TraceEventType.Information,
                                string.Format(Tracing.SendAuthnRequest, Saml20Constants.ProtocolBindings.HTTP_Artifact,
                                              idpEndpoint.Id));
