...
namespace dk.nita.saml20.protocol
{
    ...
    public abstract class Saml20AbstractEndpointHandler : AbstractEndpointHandler
    {
        ...
        public IDPEndPoint RetrieveIDP(HttpContext context)
        {
                    ...
                    if (endPoint != null)
                    {
                        if (Trace.ShouldTrace(TraceEventType.Information))
                            Trace.TraceData(TraceEventType.Information,
                                            "IDP read from Common Domain Cookie: " + cdc.PreferredIDP);

                        return endPoint;
                    }

                    AuditLogging.logEntry(Direction.IN, Operation.DISCOVER,
                                          "Invalid IdP in Common Domain Cookie, IdP not found in list of IdPs: " +
                                          cdc.PreferredIDP);
                }
            }
            ...
            {
                AuditLogging.logEntry(Direction.IN, Operation.DISCOVER,
                                      "No IdP selected in Common Domain Cookie, using default IdP: " +
                                      config.IDPEndPoints[0].Name);
                return config.IDPEndPoints[0];
            }

            // If one of the endpoints are marked with default, use that one
            IDPEndPoint defaultIdp = config.Endpoints.IDPEndPoints.Find(idp => idp.Default);
            if (defaultIdp != null)
            {
                if (Trace.ShouldTrace(TraceEventType.Information))
            ...
            if (!string.IsNullOrEmpty(config.Endpoints.idpSelectionUrl))
            {
                if (Trace.ShouldTrace(TraceEventType.Information))
                    Trace.TraceData(TraceEventType.Information,
                                    "Redirecting to idpSelectionUrl for selection of IDP: " +
                                    config.Endpoints.idpSelectionUrl);
            ...
            IDPEndPoint idpEndpoint = IDPSelectionUtil.InvokeIDPSelectionEventHandler(config.Endpoints);
            if (idpEndpoint != null)
            {
                return idpEndpoint;
            }
        ...
        {
            string errorMessage = string.Format("ErrorCode: {0}. Message: {1}.", status.StatusCode.Value,
        ...
        internal static IDPEndPointElement DetermineEndpointConfiguration(SAMLBinding defaultBinding,
                                                                          IDPEndPointElement config,
                                                                          List<IDPEndPointElement> metadata)
        {
            var result = new IDPEndPointElement();
