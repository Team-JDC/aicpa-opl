...
/**
	Modified by Dave Watson 01/07/2015
**/
	...
	if ( !($.History||false) ) {
		...
		$.History = {
			...
			setState: function ( state ) {
				...
				var History = $.History;
				console.log('History:setState: '+state);
				// Format the state
			...
			setHash: function ( hash ) {
				...
				console.log('History:setHash: '+hash);
				// Prepare hash
			...
			bind: function ( state, handler ) {
				...
				// Handle
				if ( handler ) {
				//    var justHash = new String(state);
			  //      var hashData = val.queryStringToJSON();
			...
			trigger: function ( state ) {
				...
				var justHash = History.extractJustHash(state);

				// Fire specific
				if ( typeof History.handlers.specific[justHash] !== 'undefined' ) {
					// We have specific handlers
					list = History.handlers.specific[justHash];
