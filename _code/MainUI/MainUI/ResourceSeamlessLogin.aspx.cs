...
namespace MainUI
{
    ...
    /// </summary>
    public partial class ProflitSeamlessLogin : Page
    {
        ...
        protected string GetMemberType(string site)
        {            
            
            switch (site.ToUpper())
            {
                case "LMS":
                    return ReferringSite.Lms.ToString();
                case "BIZ":
                case "CSC":
                case "FVS":
                case "PFP":
                ...
                case "ETHICSUSER":
                case "ETHICS":
                    return ReferringSite.Ethics.ToString();
                case "COSO":
                    return ReferringSite.Ethics.ToString();
                default:
                //case ReferringSite.Other:
                    return ReferringSite.Other.ToString();
            }            
        }
        ...
        protected string GetLoginEntry(string site)
        {
            switch (site.ToUpper())
            {
                case "EXAM":
                ...
                //case ReferringSite.EthicsUser:
                //case ReferringSite.Ethics:                    
                //case ReferringSite.Coso:                                    
                //case ReferringSite.Lms:
                //    return ReferringSite.Lms.ToString();
        ...
        protected  void UpdatePageVars(string url, string javascript, string redirectjs)
        {
            string urlMessage = (string.IsNullOrEmpty(url) ? "url is null" : url);
            string jsMessage = (string.IsNullOrEmpty(javascript) ? "javascript is null" : javascript);
            IEvent logEvent = new Event(EventType.Info, DateTime.Now, 1, "ResourceSeamlessLogin", "UpdatePageVars","Debug Info..", "URL:" + urlMessage + " --  Javascript:" + javascript);
            logEvent.Save(false);
            ...
            {
                logEvent = new Event(EventType.Info, DateTime.Now, 1, "ResourceSeamlessLogin", "UpdatePageVars", "Exception on redirect.","URL:"+url+ " --  Javascript:"+javascript +" --- Ex: "+ex.Message + " -- Ex2: "+ex.ToString());
                logEvent.Save(false);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string analyticsCodeLogon = string.Empty;
            string analyticsCodeLogout = string.Empty;
            try
            {
                ...
                string userName = Request.Params["hidUserName"];
                string oktaId = Request.Params["hidOktaId"];
                string roleId = Request.Params["hidOktaRoleId"];
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["prod"]))
                {
                    ...
                    {
                        referringSite = "BIZ";//ReferringSite.C2b.ToString();
                ...
                incomingParams = incomingParams + (string.IsNullOrEmpty(userName) ? string.Empty : " hidUserName: " + userName);
                incomingParams = incomingParams + (string.IsNullOrEmpty(oktaId) ? string.Empty : " hidOktaId: " + oktaId);
                incomingParams = incomingParams + (string.IsNullOrEmpty(roleId) ? string.Empty : " hidOktaRoleId: " + roleId);
                ...
                if (!string.IsNullOrEmpty(encryptedGuid))
                {
                    ...
                    if (decryptedGuid.IndexOf(',') >= 0)
                    {
                        try
                        {
                            decryptedGuid = (decryptedGuid.Split(','))[0];
                        }
                        catch (Exception ex)
                        {
                            logEvent = new Event(EventType.Info, DateTime.Now, 1, "Page_Load", "Page_Load", "Error parsing GUID", "Guid: " + decryptedGuid + " Ex Message: "+ex.Message + " -- ExExtra:"+ex.ToString());
                            logEvent.Save(false);
                        }
                    }                    
                ...
                if (retUser == null)
                {
                    ...
                    //OKT = Okta but is converted to C2b
                    if (referringSite.Equals("C2B",StringComparison.CurrentCultureIgnoreCase) || 
                        referringSite.Equals("OKT",StringComparison.CurrentCultureIgnoreCase) ||
                        referringSite.Equals("BIZ",StringComparison.CurrentCultureIgnoreCase))
                    {
                    ...
                    else if (referringSite.ToUpper() == "OKTA" || referringSite.ToUpper()=="RAVE")
                    {
                        ...
                        {
                            //Log create user record based on email 
                            retUser = new User(string.Empty, userName, ReferringSite.Rave.ToString());
                            retUser.OktaId=oktaId;
                            retUser.RoleId = roleId;
                            retUser.LogOn(Session.SessionID);
                        }
                        else
                        {
                    ...
                    {
                        string domainList = Domain;
                        
                        //Default Domain List 
                        if (string.IsNullOrEmpty(domainList))
                        {
                            logEvent = new Event(EventType.Error, DateTime.Now, 1, "ResourceSeamlessLogin.aspx", "Page_Load", "Missing Info", "Missing DefaultDomains for EXAMSNEW user");
                            logEvent.Save(false);
                        }
                        ...
                        //Create Anonymous user.
                        Guid newGuid = Guid.NewGuid();
                        retUser = new User(newGuid, ReferringSite.ExamsNew);
                    ...
                    else
                    {
                        ...
                        retUser.LogOn(Session.SessionID, Domain);
                        analyticsCodeLogon = AICPAAnalytics.LoginLogout(GetMemberType(referringSite), "examuser@exam.com", GetLoginEntry(referringSite), "login");
                        analyticsCodeLogout = AICPAAnalytics.LoginLogout(GetMemberType(referringSite), "examuser@exam.com", GetLoginEntry(referringSite), "logout");
                    }
                    #endregion Exams/etc
                ...
                {
                    //Add calculated logout call to the session variable.
                    Session[ContextManager.SESSION_ANALYTICS_LOGOUT] = analyticsCodeLogout;

                    string jsRedirect = "setTimeout(function () { window.location.href = '"+ Url+"' }, 1000);";
                    UpdatePageVars(Url,analyticsCodeLogon, jsRedirect);
                    //Response.Redirect(Url, false);
                }
            }
            catch (Exception ex)
